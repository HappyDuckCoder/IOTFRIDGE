...
public:
    ...
    long read() {
        while (digitalRead(DOUT) == HIGH); // Chờ dữ liệu sẵn sàng
...
/*
read() đọc 24-bit dữ liệu từ HX711 theo giao thức bit-banging.

read_average() lấy trung bình nhiều mẫu để giảm nhiễu.

set_scale() dùng để hiệu chỉnh (ví dụ bạn đặt 1kg lên cân và chỉnh hệ số).

tare() giúp đặt lại 0 khi cân không tải.
*/
