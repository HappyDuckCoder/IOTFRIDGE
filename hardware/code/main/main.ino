...
// Pin definitions
#define DOOR_SENSOR_PIN 2      // Digital pin for door sensor (HIGH = open, LOW = closed)
#define HX711_DOUT_PIN 4       // HX711 data pin (DT)
#define HX711_SCK_PIN 5        // HX711 clock pin (SCK)
// No additional LED or buzzer required

// System configuration
#define DOOR_ALERT_TIMEOUT 10000    // Alert after 30 seconds door open
#define DOOR_JUST_CLOSED_TIME 3000 // Consider "just closed" if < 20 seconds
#define WEIGHT_CHANGE_THRESHOLD 50.0 // 50g threshold for weight change detection
#define MAIN_LOOP_DELAY 500         // Main loop delay in ms
...
void setup() {
    ...
    Serial.println("=== Smart Fridge Monitoring System ===");
...
void loop() {
    if (!systemReady) {
        ...
        static unsigned long lastInitTry = 0;
        if (millis() - lastInitTry > 5000) {
            Serial.println("Attempting to reinitialize system...");
            if (initializeSystem()) {
                systemReady = true;
...
bool initializeSystem() {
    Serial.println("Initializing weight tracking...");
    
    if (!weightTracker.begin()) {
        Serial.println("Weight tracker initialization failed!");
        return false;
    }
    
    Serial.println("Weight tracking initialized successfully");
    
    // Initialize door state
    DoorState initialDoorState = readDoorState();
    doorTracker.setCurrentState(initialDoorState);
    
    Serial.print("Initial door state: ");
    Serial.println(initialDoorState == DOOR_OPEN ? "OPEN" : 
                  initialDoorState == DOOR_CLOSED ? "CLOSED" : "NOT_FIRM");
    
    return true;
}

DoorState readDoorState() {
    // Read door sensor (assuming HIGH = open, LOW = closed)
    bool sensorState = digitalRead(DOOR_SENSOR_PIN);
    
    // Add some debouncing by reading multiple times
    int openCount = 0;
    for (int i = 0; i < 5; i++) {
    ...
    // Consider door open if majority of readings are HIGH
    if (openCount >= 3) {
        return DOOR_OPEN;
    } else {
        return DOOR_CLOSED;
...
void handleDoorEvents() {
    DoorState currentDoorState = readDoorState();
    ...
    if (lastDoorState != currentDoorState) {
        ...
        Serial.print(lastDoorState == DOOR_OPEN ? "OPEN" : 
                    lastDoorState == DOOR_CLOSED ? "CLOSED" : "NOT_FIRM");
        Serial.print(" -> ");
        Serial.println(currentDoorState == DOOR_OPEN ? "OPEN" : 
                      currentDoorState == DOOR_CLOSED ? "CLOSED" : "NOT_FIRM");
        
        // Status logging
        if (currentDoorState == DOOR_OPEN) {
            Serial.println("Door opened - starting timer");
        } else {
            Serial.println("Door closed");
        }
    }
    
    // Check if door was just closed (for weight recalibration)
    if (doorTracker.isDoorJustClosed()) {
        Serial.println("Door just closed - recalibrating weight sensor");
        delay(2000); // Wait for vibrations to settle
...
void handleWeightEvents() {
    ...
    if (weightTracker.checkWeightChange()) {
        ...
        Serial.print("Weight change at ");
        Serial.print(millis() / 1000);
        Serial.println(" seconds");
    }
}

void handleAlerts() { //Sửa lại thêm push notification last check (min)
    // Check for door open too long alert
    if (doorTracker.isAlertNeeded()) {
        Serial.println("ALERT: Door has been open too long!");
        Serial.print("Open duration: ");
        Serial.print(doorTracker.getOpenDuration() / 1000);
        Serial.println(" seconds");
